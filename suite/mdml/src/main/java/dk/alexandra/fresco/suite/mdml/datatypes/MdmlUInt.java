 ...
 */
public interface MdmlUInt<T extends MdmlUInt> {
   ...
   */
  static <S extends MdmlUInt<S>> S sum(List<S> elements) {
    return elements.stream().reduce(MdmlUInt::add).orElse(elements.get(0));
  }

  /**
   * Compute inner product of elements.
   */
  static <S extends MdmlUInt<S>> S innerProduct(List<S> left, List<S> right) {
