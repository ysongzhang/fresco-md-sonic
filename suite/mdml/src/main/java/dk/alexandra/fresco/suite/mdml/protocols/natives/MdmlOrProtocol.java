 ...
 */
public class MdmlOrProtocol<PlainT extends MdmlCompUInt<?, ?, PlainT>> extends
  ...
  /**
   * Creates new {@link MdmlOrProtocol}.
   *
   * @param left left factor
   * @param right right factor
   */
  public MdmlOrProtocol(DRes<SInt> left, DRes<SInt> right) {
    this.left = left;
    this.right = right;
  }
  ...
  public EvaluationStatus evaluate(int round,MdmlResourcePool<PlainT> resourcePool,
    ...
    if (round == 0) {
      ...
      MdmlSIntBoolean<PlainT> anded = tripleProduct
      ...
      MdmlSIntBoolean<PlainT> xored = factory.toMdmlSIntBoolean(left)
          .xor(factory.toMdmlSIntBoolean(right));
      result = anded.xor(xored);
