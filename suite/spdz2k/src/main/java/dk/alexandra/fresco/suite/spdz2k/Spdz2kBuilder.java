...
public class Spdz2kBuilder<PlainT extends CompUInt<?, ?, PlainT>> implements
    ...
    return new Numeric() {
      ...
      public DRes<SInt> input(BigInteger value, int inputParty) {
        ...
        if (builder.getBasicNumericContext().getNoOfParties() <= 2) {
          return builder.append(new Spdz2kTwoPartyInputProtocol<>(val, inputParty));
        } else {
          return builder.seq(new InputComputationSpdz2k<>(val, inputParty));
        }
      }
  ...
  public RealLinearAlgebra createRealLinearAlgebra(ProtocolBuilderNumeric builder) {
    return new Spdz2kFixedLinearAlgebra(builder);
  }
