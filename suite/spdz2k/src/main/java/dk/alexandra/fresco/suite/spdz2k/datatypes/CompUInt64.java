...
public class CompUInt64 implements CompUInt<UInt32, UInt32, CompUInt64> {
  ...
  public CompUInt64 shiftRightLowOnly(int n) {
    ...
    int low = toInt();
    return new CompUInt64((((value >>> 32) << 32) | UInt.toUnLong(low >>> n)));
  }

  public CompUInt64 shiftRightLowOnlySigned(int n) {
    if (n >= Integer.SIZE) {
      throw new IllegalArgumentException(
              "Shift step must be less than " + Integer.SIZE + " but was " + n);
    }
  ...
  public BigInteger toSignedBigInteger() {
    ...
    if (uIntInfo.compareTo(BigInteger.ONE.shiftLeft(31)) > 0) {
