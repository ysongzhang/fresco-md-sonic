...
public class Spdz2kDummyDataSupplier<
  ...
  private Matrix<Spdz2kSIntArithmetic<PlainT>> getNextMatrixZeroShare(int height, int width) {
    return new Matrix<>(height, width, i -> {
      ArrayList<Spdz2kSIntArithmetic<PlainT>> row = new ArrayList<>(width);
      for (int j = 0; j < width; j++) {
        row.add(toSpdz2kSInt(supplier.getNextZeroShare()));
      }
      return row;
    });
  }
  ...
  @Override
  public Spdz2kMatrixTriple<PlainT, Spdz2kSIntArithmetic<PlainT>> getNextMatrixTripleShares(int n1, int n2, int n3) {
    Matrix<Spdz2kSIntArithmetic<PlainT>> left = getNextMatrixZeroShare(n1, n2);
    Matrix<Spdz2kSIntArithmetic<PlainT>> right = getNextMatrixZeroShare(n2, n3);
    Matrix<Spdz2kSIntArithmetic<PlainT>> product = getNextMatrixZeroShare(n1, n3);
    return new Spdz2kMatrixTriple<>(left, right, product);
  }
