 ...
 *
 * SPDZ2k with k=1, representing Boolean values as described in [DEF+19], requires s+1 bits of space for storage (k=s).
 * For the CompUInt64Bit type, the last s+1 bits are used, with the last bit of LowT storing the actual bit information.
 * For the CompUInt128Bit type, the first s+1 bits are used, with the first bit of LowT storing the actual bit information.
 *
 * In Spdz2kSIntBoolean, PlainT is either CompUInt64 or CompUInt128, containing two variables. Among them, share should
 * be of type CompUInt64Bit or CompUInt128Bit, while macShare is of type PlainT.
 */
