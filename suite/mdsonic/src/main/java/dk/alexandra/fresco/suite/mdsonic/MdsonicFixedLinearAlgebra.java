...
import dk.alexandra.fresco.suite.mdsonic.protocols.natives.ASS.MdsonicFixedMatrixProductProtocolASS;
import dk.alexandra.fresco.suite.mdsonic.protocols.natives.MSS.MdsonicFixedMatrixProductProtocolMSS;
...
public class MdsonicFixedLinearAlgebra implements RealLinearAlgebra {
...
//  private long then = 0;
  ...
  public MdsonicFixedLinearAlgebra(ProtocolBuilderNumeric builder, int precision) {
    this.builder = builder;
    this.precision = precision;
    this.useMaskedEvaluation = false;
  }

  public MdsonicFixedLinearAlgebra(ProtocolBuilderNumeric builder, int precision, boolean useMaskedEvaluation) {
    this.builder = builder;
    this.precision = precision;
    this.useMaskedEvaluation = useMaskedEvaluation;
  }

  public MdsonicFixedLinearAlgebra(ProtocolBuilderNumeric builder) {
    this(builder, builder.getRealNumericContext().getPrecision());
  }

  public MdsonicFixedLinearAlgebra(ProtocolBuilderNumeric builder, boolean useMaskedEvaluation) {
...
//      now = System.currentTimeMillis();
      Matrix<DRes<SReal>> matrix =
          new Matrix<>(a.getHeight(), a.getWidth(), i -> new ArrayList<>(a.getRow(i).stream()
              .map(e -> par.realNumericMdsonic().input(e, inputParty)).collect(Collectors.toList())));
      return () -> matrix;
    });
  }
  ...
  public DRes<Vector<DRes<SReal>>> input(Vector<BigDecimal> a, int inputParty) {
    return builder.par(par -> {
      Vector<DRes<SReal>> matrix = a.stream().map(e -> par.realNumericMdsonic().input(e, inputParty))
  ...
  public DRes<Matrix<DRes<BigDecimal>>> openMatrix(DRes<Matrix<DRes<SReal>>> a) {
    return builder.par(par -> {
      Matrix<DRes<BigDecimal>> matrix = new Matrix<>(a.out().getHeight(), a.out().getWidth(),
          i -> new ArrayList<>(a.out().getRow(i).stream().map(e -> par.realNumericMdsonic().open(e))
  ...
  public DRes<Vector<DRes<BigDecimal>>> openVector(DRes<Vector<DRes<SReal>>> a) {
    return builder.par(par -> {
      Vector<DRes<BigDecimal>> vector = a.out().stream().map(e -> par.realNumericMdsonic().open(e))
          .collect(Collectors.toCollection(Vector::new));
      return () -> vector;
    });
  }
  ...
  public DRes<Matrix<DRes<SReal>>> add(DRes<Matrix<DRes<SReal>>> a, DRes<Matrix<DRes<SReal>>> b) {
    return builder.par(par -> {
      return entrywiseBinaryOperator(par, a.out(), b.out(),
          (builder, x) -> builder.realNumericMdsonic().add(x.getFirst(), x.getSecond()));
    });
  }
  ...
  public DRes<Matrix<DRes<SReal>>> sub(DRes<Matrix<DRes<SReal>>> a, DRes<Matrix<DRes<SReal>>> b) {
    return builder.par(par -> {
      return entrywiseBinaryOperator(par, a.out(), b.out(),
          (builder, x) -> builder.realNumericMdsonic().sub(x.getFirst(), x.getSecond()));
    });
  }
  ...
  public DRes<Matrix<DRes<SReal>>> mult(DRes<Matrix<DRes<SReal>>> a, Matrix<BigDecimal> b) {
    if (useMaskedEvaluation) {
      ...
      return builder.seq(seq -> {
        return mult(seq, a.out(), b, (scope, x) -> scope.realAdvancedMdsonic()
  ...
  public DRes<Matrix<DRes<SReal>>> mult(Matrix<BigDecimal> a, DRes<Matrix<DRes<SReal>>> b) {
    if (useMaskedEvaluation) {
      ...
      return builder.seq(seq -> {
        return mult(seq, a, b.out(), (builder, x) -> builder.realAdvancedMdsonic()
  ...
  public DRes<Matrix<DRes<SReal>>> mult(DRes<Matrix<DRes<SReal>>> a, DRes<Matrix<DRes<SReal>>> b, boolean useMatrixTriple) {
    if (useMaskedEvaluation) {
      return builder.seq(seq -> {
        Matrix<DRes<SReal>> aOut = a.out();
        Matrix<DRes<SReal>> bOut = b.out();
        DRes<MatrixTruncationPair> matrixTruncationPairDRes = seq.advancedNumericMdsonic().generateMatrixTruncationPair(precision, aOut.getHeight(), bOut.getWidth());
        return seq.append(new MdsonicFixedMatrixProductProtocolMSS<>(a, b, matrixTruncationPairDRes, precision));
      });
    } else {
      if (useMatrixTriple) {
        return builder.seq(seq -> {
          return seq.append(new MdsonicFixedMatrixProductProtocolASS<>(a, b));
        }).seq((seq, matrix) -> {
          // truncation
        ...
        return builder.seq(seq -> {
          return mult(seq, a.out(), b.out(),
                  (builder, x) -> builder.realAdvancedMdsonic().innerProduct(x.getFirst(), x.getSecond()));
        });
      }
    }
  }
    ...
    return builder.par(par -> {
      Matrix<DRes<SReal>> result = new Matrix<>(input.getHeight(), input.getWidth(), i -> {
        ...
        for (int j = 0; j < input.getWidth(); j++) {
          DRes<SInt> truncated = par.advancedNumericMdsonic().truncate(rowInput.get(j), precision);
  ...
  public DRes<Matrix<DRes<SReal>>> scale(BigDecimal s, DRes<Matrix<DRes<SReal>>> a) {
    return builder.par(par -> {
      return scale(par, s, a.out(),
          (builder, x) -> builder.realNumericMdsonic().mult(x.getFirst(), x.getSecond()));
    });
  }
  ...
  public DRes<Matrix<DRes<SReal>>> scale(DRes<SReal> s, Matrix<BigDecimal> a) {
    return builder.par(par -> {
      return scale(par, s, a,
          (builder, x) -> builder.realNumericMdsonic().mult(x.getSecond(), x.getFirst()));
    });
  }
  ...
  public DRes<Vector<DRes<SReal>>> vectorMult(DRes<Matrix<DRes<SReal>>> a, Vector<BigDecimal> v) {
    return builder.par(par -> {
      return vectorMult(par, a.out(), v, (builder, x) -> builder.realAdvancedMdsonic()
          .innerProductWithPublicPart(x.getSecond(), x.getFirst()));
    });
  }
  ...
  public DRes<Vector<DRes<SReal>>> vectorMult(Matrix<BigDecimal> a, DRes<Vector<DRes<SReal>>> v) {
    return builder.par(par -> {
      return vectorMult(par, a, v.out(), (builder, x) -> builder.realAdvancedMdsonic()
          .innerProductWithPublicPart(x.getFirst(), x.getSecond()));
    });
  }
  ...
  @Override
  public DRes<Matrix<DRes<SReal>>> replicate(DRes<Matrix<DRes<SReal>>> vectorShare, int n){
    return builder.par(par -> {
      if (vectorShare.out().getWidth() != 1) {
        throw new IllegalArgumentException("The input matrix must be a column vector");
      }
      Matrix<DRes<SReal>> res = new Matrix<>(vectorShare.out().getHeight(), n, i -> {
        ArrayList<DRes<SReal>> row = new ArrayList<>(n);
        DRes<SReal> value = vectorShare.out().getRow(i).get(0);
        for (int j = 0; j < n; j++) {
          row.add(value);
        }
        return row;
