 ...
 */
public class MdsonicCompUInt128 implements MdsonicCompUInt<MdsonicUInt64, MdsonicUInt64, MdsonicCompUInt128> {

  private static final MdsonicCompUInt128 ONE = new MdsonicCompUInt128(1);
  private static final MdsonicCompUInt128 ZERO = new MdsonicCompUInt128(0);
  protected final long high;  // 64 bits
  protected final int mid;  // 32 bits
  final int low;

  /**
   * Creates new {@link MdsonicCompUInt128}. <p>Do <b>not</b> pad bytes by default.</p>
   *
   * @param bytes bytes interpreted in big-endian order.
   */
  public MdsonicCompUInt128(byte[] bytes) {
    this(bytes, false);
  }

  /**
   * Creates new {@link MdsonicCompUInt128}.
   *
   * @param bytes bytes interpreted in big-endian order.
   * @param requiresPadding indicates if the bytes need to be padded up to 16 bytes.
   */
  public MdsonicCompUInt128(byte[] bytes, boolean requiresPadding) {
    byte[] padded = requiresPadding ? MdsonicCompUInt.pad(bytes, 128) : bytes;
    if (padded.length == 8) {
  ...
  /**
   * Creates new {@link MdsonicCompUInt128} from {@link BigInteger}.
   */
  public MdsonicCompUInt128(BigInteger value) {
    this(value.shiftRight(64).longValue(), value.shiftRight(32).intValue(), value.intValue());
  }

  public MdsonicCompUInt128(BigInteger value, boolean lowOnly) {
    this(value.longValue());
  }

  MdsonicCompUInt128(long high, int mid, int low) {
    this.high = high;
    this.mid = mid;
    this.low = low;
  }

  MdsonicCompUInt128(MdsonicUInt64 value) {
    this(value.toLong());
  }

  MdsonicCompUInt128(long value) {
    this.high = 0;
    this.mid = (int) (value >>> 32);
    this.low = (int) value;
  }

  MdsonicCompUInt128(MdsonicCompUInt128 other) {
  ...
  public MdsonicCompUInt128 add(MdsonicCompUInt128 other) {
    ...
    return new MdsonicCompUInt128(newHigh, (int) newMid, (int) newLow);
  }

  @Override
  public MdsonicCompUInt128 multiply(MdsonicCompUInt128 other) {
    long thisLowAsLong = MdsonicUInt.toUnLong(this.low);
    long thisMidAsLong = MdsonicUInt.toUnLong(this.mid);
    long otherLowAsLong = MdsonicUInt.toUnLong(other.low);
    long otherMidAsLong = MdsonicUInt.toUnLong(other.mid);
    ...
    int m2 = (int) m1;
    long newMid = MdsonicUInt.toUnLong(m2) + (t4 & 0xffffffffL);

    long newHigh = (t2 >>> 32)
        + t3
        + (t4 >>> 32)
        + t5
        + (MdsonicUInt.toUnLong(t6) << 32)
        + t7
        + (MdsonicUInt.toUnLong(t8) << 32)
        + (m1 >>> 32)
        + (newMid >>> 32);
    return new MdsonicCompUInt128(newHigh, (int) newMid, (int) t1);
  }
...
//    return new CompUInt128(newHigh, (int) newMid, (int) newLow);
  ...
  public MdsonicCompUInt128 negate() {
    return new MdsonicCompUInt128(~high, ~mid, ~low).add(ONE);
  }
  ...
  public BigInteger toSignedBigInteger() {
    // Extract 64 valid message bits
    BigInteger uIntInfo = new BigInteger(1, toByteArray64());
    // Whether the MSB == 1
    if (uIntInfo.compareTo(BigInteger.ONE.shiftLeft(63)) > 0) { // negative number
  ...
  @Override
  public MdsonicUInt64 getLeastSignificant() {
    return new MdsonicUInt64(toLong());
  }

  @Override
  public MdsonicUInt64 getMostSignificant() {
    return new MdsonicUInt64(high);
  }

  @Override
  public MdsonicUInt64 getLeastSignificantAsHigh() {
    return getLeastSignificant();
  }

  @Override
  public MdsonicCompUInt128 shiftLeftSmall(int n) {
    if (n >= Long.SIZE) {
    ...
    long midAndLow = (MdsonicUInt.toUnLong(mid) << 32) | MdsonicUInt.toUnLong(low);
    long newHigh = (high << n) | (midAndLow >>> nInv);
    long midAndLowShifted = midAndLow << n;
    return new MdsonicCompUInt128(newHigh, (int) (midAndLowShifted >>> 32), (int) midAndLowShifted);
  }

  @Override
  public MdsonicCompUInt128 shiftRightSmall(int n) {
    if (n >= Long.SIZE) {
      throw new IllegalArgumentException(
          "Shift step must be less than " + Long.SIZE + " but was " + n);
    }
  ...
  @Override
  public MdsonicCompUInt128 shiftRightLowOnly(int n) {
    if (n >= Long.SIZE) {
      throw new IllegalArgumentException(
          "Shift step must be less than " + Long.SIZE + " but was " + n);
    }
    ...
    long midAndLow = ((MdsonicUInt.toUnLong(mid) << 32) | MdsonicUInt.toUnLong(low)) >>> n;
    return new MdsonicCompUInt128(high, (int) (midAndLow >>> 32), (int) midAndLow);
  }

  public MdsonicCompUInt128 shiftRightLowOnlySigned(int n) {
    if (n >= Long.SIZE) {
      throw new IllegalArgumentException(
              "Shift step must be less than " + Long.SIZE + " but was " + n);
    }
  ...
  @Override
  public long toLong() {
    return (MdsonicUInt.toUnLong(this.mid) << 32) + MdsonicUInt.toUnLong(this.low);
  }
  ...
  @Override
  public MdsonicCompUInt128 shiftLowIntoHigh() {
    return new MdsonicCompUInt128(toLong(), 0, 0);
  }

  @Override
  public MdsonicCompUInt128 clearAboveBitAt(int bitPos) {
    if (bitPos < Integer.SIZE) {
      int mask = ~(0x80000000 >> (31 - bitPos));
      return new MdsonicCompUInt128(0L, 0, low & mask);
    } else if (bitPos < Long.SIZE) {
      int mask = ~(0x80000000 >> (63 - bitPos));
      return new MdsonicCompUInt128(0L, mid & mask, low);
    } else {
      long mask = ~(0x8000000000000000L >> (127 - bitPos));
      return new MdsonicCompUInt128(high & mask, mid, low);
    }
  }
  ...
  public MdsonicCompUInt128 multiplyByBit(int value) {
    return multiply(new MdsonicCompUInt128(0L, 0, value));
  }

  @Override
  public MdsonicCompUInt128 clearHighBits() {
    return new MdsonicCompUInt128(0, mid, low);
  }

  @Override
  public MdsonicCompUInt128 toBitRep() {
    return new MdsonicCompUInt128Bit(
        (high << 63) + (MdsonicUInt.toUnLong(mid) << 31) + (MdsonicUInt.toUnLong(low) >>> 1),
        low << 31,
        0);
  }
  ...
  @Override
  public MdsonicCompUInt128 testBitAsUInt(int bit) {
