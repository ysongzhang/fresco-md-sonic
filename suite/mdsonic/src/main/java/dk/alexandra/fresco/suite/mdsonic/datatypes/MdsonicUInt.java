 ...
 */
public interface MdsonicUInt<T extends MdsonicUInt> {
   ...
   */
  static <S extends MdsonicUInt<S>> S sum(List<S> elements) {
    return elements.stream().reduce(MdsonicUInt::add).orElse(elements.get(0));
  }

  /**
   * Compute inner product of elements.
   */
  static <S extends MdsonicUInt<S>> S innerProduct(List<S> left, List<S> right) {
