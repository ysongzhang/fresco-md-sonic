 ...
 */
public interface RealNumericMdsonic extends ComputationDirectory {
   ...
   */
  DRes<SBool> leq(DRes<SReal> x, DRes<SReal> y);

  DRes<SBool> lt(DRes<SReal> x, DRes<SReal> y);

  DRes<SBool> equals(DRes<SReal> x, DRes<SReal> y);

  DRes<SBool> zero(DRes<SReal> x, int bitLength);

  DRes<SReal> relu(DRes<SReal> input);
}
