...
public interface RealLinearAlgebra extends ComputationDirectory {
   ...
   * @return A deferred result computing a*b
   */
  default DRes<Matrix<DRes<SReal>>> mult(DRes<Matrix<DRes<SReal>>> a, DRes<Matrix<DRes<SReal>>> b) {
    return mult(a, b, false);
  }

  DRes<Matrix<DRes<SReal>>> mult(DRes<Matrix<DRes<SReal>>> a, DRes<Matrix<DRes<SReal>>> b, boolean useMatrixTriple);

  /**
   * Multiply a matrix to a vector.
   * 
   * @param a First secret value
   * @param v Second secret value
  ...
  /**
   * Replicate a column vector to n columns.
   *
   * @param vectorShare The column vector.
   * @param n The number of replication.
   * @return The matrix of n column vectors.
   */
  DRes<Matrix<DRes<SReal>>> replicate(DRes<Matrix<DRes<SReal>>> vectorShare, int n);
