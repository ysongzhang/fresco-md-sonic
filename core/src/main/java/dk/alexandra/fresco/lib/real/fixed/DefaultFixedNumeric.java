...
public class DefaultFixedNumeric implements RealNumeric {
  ...
  public DRes<SInt> lt(DRes<SReal> x, DRes<SReal> y) {
    return builder.seq(seq -> {
      SFixed floatX = (SFixed) x.out();
      SFixed floatY = (SFixed) y.out();
      return seq.comparison().compareLT(floatX.getSInt(), floatY.getSInt());
    });
  }

  public DRes<SInt> equals(DRes<SReal> x, DRes<SReal> y) {
    return builder.seq(seq -> {
      SFixed floatX = (SFixed) x.out();
      SFixed floatY = (SFixed) y.out();
      return seq.comparison().equals(floatX.getSInt(), floatY.getSInt());
    });
  }

  public DRes<SInt> zero(DRes<SReal> x, int bitlength) {
    return builder.seq(seq -> {
      SFixed floatX = (SFixed) x.out();
      return seq.comparison().compareZero(floatX.getSInt(), bitlength);
    });
  }
