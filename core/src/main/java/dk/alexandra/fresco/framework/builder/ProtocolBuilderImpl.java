...
public abstract class ProtocolBuilderImpl<BuilderT extends ProtocolBuilderImpl<BuilderT>>
   ...
   */
  public <T> DRes<T> append(NativeProtocol<T, ?> nativeProtocol) {  // append方法会自动在nativeProtocol输出结果上套上DRes
   ...
   */
  public <R> BuildStep<Void, BuilderT, R> seq(Computation<R, BuilderT> function) {  // the seq function can remove DRes structure.
