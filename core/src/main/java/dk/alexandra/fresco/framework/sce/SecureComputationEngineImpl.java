...
public class SecureComputationEngineImpl
  ...
  private <OutputT> DRes<OutputT> evalApplication(Application<OutputT, BuilderT> application,
    ...
    try{
      NetworkLoggingDecorator loggingDecorator = (NetworkLoggingDecorator) network;
      logger.info("The application {} finished evaluation with communication {}.", application, loggingDecorator.getLoggedValues());
    } catch (Exception e) {
      throw e;
    }
    application.close();
