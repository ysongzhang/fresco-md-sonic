...
public class MdsonicActivationFunctionsBench implements ActivationFunctionsBench {

  private ProtocolBuilderNumeric builder;

  public MdsonicActivationFunctionsBench(ProtocolBuilderNumeric builder) {
  ...
  public DRes<Matrix<DRes<SReal>>> relu(DRes<Matrix<DRes<SReal>>> v) {
    return builder.par(par -> {
      return par.seq(new ReluComputationBench(v));
    });
  }
